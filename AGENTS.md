...
| 协议 | MIT |
| 立项时间 | 2026-06-04 |
| 当前版本 | v0.100.0(✨ 执行记录抽屉化:会话头部新增「执行」胶囊,SSH 静默执行记录迁入工具抽屉(去重置顶/点击展开收起/纵向滚动/一键清空),移除右下角浮层及其拖动重排;DiffBlock 改双栏修改前后对照(`-`/`+` 着色、LCS 行对齐、相邻增删折叠、两列联动滚动);修复干净检出上 vendor 全量构建 TS2307 崩溃(gen:typert 源码态自举挂入 build:lib:host 前);修复 Modal 弹面 l2 滚动条未重绑。) |

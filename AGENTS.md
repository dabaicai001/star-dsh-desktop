...
| 主分支 | `main` |
| 协议 | MIT |
| 当前版本 | v0.118.3(**Git 工作台变更列表丢失未暂存改动**:`git status --porcelain=v1 -z` 输出以空格开头的记录(未暂存修改)被 `trim()` 误删行首状态位,导致「变更」Tab 漏列未暂存文件;改为仅剥离尾随 NUL/换行,保留行首空格。) |

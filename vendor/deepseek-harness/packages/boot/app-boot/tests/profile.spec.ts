...
import {
  existsSync, lstatSync, mkdirSync, mkdtempSync, readdirSync, readFileSync, readlinkSync, realpathSync, rmSync,
...
describe('healProfilesModuleFallback', () => {
  ...
  it('quarantines a foreign fallback file or directory and rebuilds the link', async () => {
    const anchor = stageInstallation({})
    for (const kind of ['file', 'directory']) {
      ...
      if (kind === 'directory') {
        mkdirSync(entry)
        writeFileSync(join(entry, 'stray.txt'), 'user data that must not be deleted')
      } else {
        writeFileSync(entry, 'user data that must not be deleted')
      }
      await healProfilesModuleFallback({ installAnchor: anchor, home })
      // The foreign entry is moved aside (preserved) and the managed link is rebuilt.
      expect(lstatSync(entry).isSymbolicLink()).toBe(true)
      const backups = readdirSync(dirname(entry)).filter(name => name.startsWith('dsh-app.dshbak-'))
      expect(backups).toHaveLength(1)
      const stray = kind === 'directory'
        ? join(home, 'profiles', 'node_modules', backups[0]!, 'stray.txt')
        : join(home, 'profiles', 'node_modules', backups[0]!)
      expect(readFileSync(stray, 'utf8')).toBe('user data that must not be deleted')
      // A second heal keeps the correct link and does not accumulate more backups.
      await healProfilesModuleFallback({ installAnchor: anchor, home })
      expect(readdirSync(dirname(entry)).filter(name => name.startsWith('dsh-app.dshbak-'))).toHaveLength(1)
      expect(lstatSync(entry).isSymbolicLink()).toBe(true)
    }
  })
  ...
  it('quarantines foreign packaged fallback directories with valid or invalid metadata', async () => {
    const anchor = stageInstallation({ 'bundle-a': { patch: '[]\n' } })
    Object.defineProperty(process, 'pkg', { configurable: true, value: {} })
    try {
      for (const metadata of ['{}', '{']) {
        ...
        await healProfilesModuleFallback({ installAnchor: anchor, home })
        // The foreign directory is moved aside (preserved) and the dsh-managed proxy is rebuilt.
        expect(lstatSync(proxy).isDirectory()).toBe(true)
        expect(JSON.parse(readFileSync(join(proxy, 'package.json'), 'utf8')))
          .toMatchObject({ name: 'bundle-a' })
        const backups = readdirSync(dirname(proxy)).filter(name => name.startsWith('bundle-a.dshbak-'))
        expect(backups).toHaveLength(1)
        expect(readFileSync(join(home, 'profiles', 'node_modules', backups[0]!, 'package.json'), 'utf8'))
          .toBe(metadata)
      }
    } finally {
      delete (process as NodeJS.Process & { pkg?: unknown }).pkg

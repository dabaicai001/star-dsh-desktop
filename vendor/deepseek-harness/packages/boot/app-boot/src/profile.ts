...
/** Ensure `link` is a symlink to `target`, replacing a wrong link, a dsh-managed packaged proxy, or a foreign entry; a foreign entry is quarantined, not fatal. */
...
function ensureModuleProxy(
  ...
  if (stat !== undefined) {
    ...
    if (existing?.dsh?.moduleFallback?.targets === undefined) {
      // Foreign entry: quarantine it like ensureSymlink does — fail-loud here
      // permanently bricks the packaged desktop app.
      quarantineForeignEntry(link)
    } else {

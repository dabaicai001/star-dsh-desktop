...
keyless CI 工作流（`.github/workflows/ci.yml`）将独立门禁分组到若干宽粒度 lane，并在受支持的 Node 版本上运行一组较小的兼容性检查；本 fork 未随源码引入该工作流，其 CI 保留在外层仓库的 `.github/workflows/` 中，其中 `release.yml` 由 `v*.*.*` 标签触发 Release 构建。产物消费方在各自 lane 内等待一次 build。单独的真实 API 工作流按其配置的 worker 上限运行 `pnpm run test:e2e`。当前门禁和 job 清单以 [scripts/run-gates.ts](../scripts/run-gates.ts) 和工作流文件为准。

...
rem run obscura-render tests with the paint feature, using the same
...
if exist "%USERPROFILE%\.cargo\bin\cargo-nextest.exe" (
  cargo nextest run --features paint -p obscura-render --no-fail-fast
) else (
  cargo test --features paint -p obscura-render
)
